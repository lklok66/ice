                       ...
                       inTargetBuildContext:(PBXTargetBuildContext*)context
{
    ...
    if(conf.sdk)
    {
        ...
        if(conf.cpp)
        {
            ...
            {
                options = [NSArray arrayWithObjects:@"-liconv", @"-lbz2", @"-lcrypto", @"-lssl", @"-lIceCpp", nil];
            }
            else if([[context expandedValueForString:@"$(PLATFORM_NAME)"] isEqualToString:@"iphoneos"])
            {
                options = [NSArray arrayWithObjects:@"-lstdc++", @"-lIceCpp", nil];
            }
            else // iPhoneSimulator
            {
                options = [NSArray arrayWithObjects:@"-lstdc++", @"-lIceCpp", 
        ...
        else
        {
            if([[context expandedValueForString:@"$(PLATFORM_NAME)"] isEqualToString:@"macosx"])
            {
                options = [NSArray arrayWithObjects:@"-ObjC", @"-lstdc++", @"-liconv", @"-lbz2", @"-lcrypto", 
                                   @"-lssl", @"-lIceObjC", nil];
            }
            else if([[context expandedValueForString:@"$(PLATFORM_NAME)"] isEqualToString:@"iphoneos"])
            {
