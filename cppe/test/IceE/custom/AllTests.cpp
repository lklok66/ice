...
public:
    ...
    {
        test(distance(out.first, out.second) == static_cast<Ice::Int>(_in.size()));
        test(distance(ret.first, ret.second) == static_cast<Ice::Int>(_in.size()));
...
#ifdef ICEE_HAS_AMI
    tprintf("testing alternate sequences with AMI... "); fflush(stdout);
