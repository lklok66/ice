...
public class SliceTask extends org.apache.tools.ant.Task
{
    ...
    //
    //  1. Through the icee.home property
    //  2. Through the ICEE_HOME environment variable.
    //
    ...
    getIceHome()
    {
        if(_iceHome == null)
        {
            if(getProject().getProperties().containsKey("icee.home"))
            {
                _iceHome = (String)getProject().getProperties().get("icee.home");
            }
            else 
            {
                _iceHome = getEnvironment("ICEE_HOME");
            }
        }
        return _iceHome;
    }
