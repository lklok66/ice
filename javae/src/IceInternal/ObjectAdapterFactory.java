...
public final class ObjectAdapterFactory
{
    ...
    createObjectAdapter(String name, String endpoints, Ice.RouterPrx router)
    {
        ...
        if(name.length() == 0 && (endpoints.length() != 0 || router != null))
        {
            ...
            ex.unsupportedFeature = "Cannot configure endpoints or router with nameless object adapter";
            throw ex;
        }
        ...
        if(name.length() == 0)
        {
            ...
            adapter = new Ice.ObjectAdapter(_instance, _communicator, this, uuid, "", null, true);
            _adapters.put(uuid, adapter);
        }
        else
        {
            adapter = new Ice.ObjectAdapter(_instance, _communicator, this, name, endpoints, router, false);
