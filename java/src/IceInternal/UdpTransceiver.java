...
final class UdpTransceiver implements Transceiver
{
    class ReadThread extends TransceiverReadThread
    {
        protected void read(Buffer buf)
        {
            ...
            {
                //
                // We log a warning here because this is the server side
                // -- without
                // the
            ...
            while(true)
            {
                try
                {
                    ...
                    {
                        //
                        // If we must connect, then we connect to the
                        // first peer that sends us a packet.
                        //
    ...
    class WriteThread extends TransceiverWriteThread
    {
        protected void write(Buffer buf)
        {
            ...
            {
                //
                // We don't log a warning here because the client gets
                // an exception anyway.
                //
                throw new Ice.DatagramLimitException();
            }
    ...
    synchronized public boolean initialize(AsyncCallback asyncCallback)
    {
        if(_readThread == null)
        {
            StringBuffer desc = new StringBuffer();
            desc.append(_addr.getAddress());
            desc.append(':');
            desc.append(_addr.getPort());
            
            _readThread = new ReadThread();
            _readThread.setName("UdpTransceiverReadThread-" + desc);
            _readThread.start();
            
            _writeThread = new WriteThread();
            _writeThread.setName("UdpTransceiverWriteThread-" + desc);
            _writeThread.start();
        }
        return true;
    }
    ...
    private synchronized void setBufSize(Instance instance)
    {
        ...
        for(int i = 0; i < 2; ++i)
        {
            ...
            if(sizeRequested != dfltSize)
            {
                ...
                //
                // Warn if the size that was set is less than the requested
                // size.
                //
    ...
    //
    // The NIO classes do not support multicast, at least not directly. This method works around that limitation by
    // using reflection to configure the file descriptor of a DatagramChannel for multicast operation. Specifically, an
    // instance of java.net.PlainDatagramSocketImpl is used to (temporarily) wrap the channel's file descriptor.
    //
    private void configureMulticast(java.net.SocketAddress group, String interfaceAddr, int ttl)
    {
        try
        {
            java.lang.reflect.Constructor c = Class.forName("java.net.PlainDatagramSocketImpl").getDeclaredConstructor(
                    (Class[]) null);
            c.setAccessible(true);
            java.net.DatagramSocketImpl socketImpl = (java.net.DatagramSocketImpl) c.newInstance((Object[]) null);

            //
            // We have to invoke the protected create() method on the
            // PlainDatagramSocketImpl object so
            // that this hack works properly when IPv6 is enabled on Windows.
            //
            java.lang.reflect.Method m = Class.forName("java.net.PlainDatagramSocketImpl").getDeclaredMethod("create",
                    (Class[]) null);
    ...
    //
    // The maximum IP datagram size is 65535. Subtract 20 bytes for the IP
    // header and 8 bytes for the UDP header
