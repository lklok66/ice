...
if not os.path.exists(os.path.join(distDir, "src", "windows", "LICENSE.rtf")):
    print sys.argv[0] + ": you must run makejsbindist.py from the dist-" + version + " directory created by makedist.py"
    sys.exit(1)

print "Building IceJS " + version + " binary distribution (" + platform.getPackageName("IceJS", version) + ".tar.gz)"
...
srcDir = os.path.join(buildRootDir, "IceJS-" + version + "-src")
buildDir = os.path.join(buildRootDir, "IceJS-" + version)
...
for f in [ 'README', 'SOURCES' ]:
    copy(os.path.join(distDir, "src", "js", f), os.path.join(buildDir, f))
print "ok"

#
# Everything should be clean now, we can create the binary distribution archive
# 
platform.createArchive(cwd, "IceJS", buildRootDir, distDir, version, quiet)
