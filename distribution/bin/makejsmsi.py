...
sourceArchive = os.path.join(iceBuildHome, "IceJS-%s.zip" % version)
demoArchive = os.path.join(iceBuildHome, "IceJS-%s-demos.zip" % version)

distFiles = os.path.join(iceBuildHome, "distfiles-%s" % version)

iceInstallerFile = os.path.join(distFiles, "src", "windows" , "IceJS.aip")
...
installerDir = os.path.join(iceBuildHome, "installer", "IceJS-%s" % version)
installerdSrcDir = os.path.join(iceBuildHome, "installer", "IceJS-%s-src" % version)
installerDemoDir = os.path.join(iceBuildHome, "installer", "IceJS-%s-demos" % version)
...
# docs dir
#
docsDir = os.path.join(distFiles, "src", "js")
...
<PathVariables Application="Advanced Installer" Version="10.3">
  <Var Name="ICE_BUILD_HOME" Path="%ICE_BUILD_HOME%" Type="2" ContentType="2"/>
    ...
    shutil.move(os.path.join(os.path.dirname(iceInstallerFile), "IceJS.msi"), \
                             os.path.join(iceBuildHome, "IceJS-%s.msi" % version))
