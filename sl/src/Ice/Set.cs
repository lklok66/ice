...
namespace IceUtil
{
    public class Set : ICollection //, ICloneable
    {
        ...
        public Set(int capacity, double loadFactor)
        {
            if(loadFactor <= 0)
            {
                throw new ArgumentOutOfRangeException("loadFactor", "load factor must be greater than 0.0");
            }
            if(loadFactor > 1)
            {
                throw new ArgumentOutOfRangeException("loadfactor", "load factor must not be greater than 1.0");
            }
