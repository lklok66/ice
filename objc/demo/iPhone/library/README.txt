...
If you intend to run this demo using a local server instead of
the public one provided by ZeroC, you will need to modify the SSL
configurations of Glacier2 and the server to use new certificates.
iOS 5 no longer supports X.509 certificates that use MD5 hashes.
Since the certificates that are included in the Ice distribution use
MD5 hashes, the iPhone client will not be able to connect to a local
server unless you replace the certificates with new ones that are
included in your Ice Touch distribution.
